...
public final class MatrixUtil {
  ...
  public static void buildMatrix(BitArray dataBits, ErrorCorrectionLevel ecLevel, int version,
      int maskPattern, ByteMatrix matrix) throws WriterException {//原生方法
    clearMatrix(matrix);
    ...
    embedDataBits(dataBits, maskPattern, matrix);//将数据比特进行掩码并填充矩阵
  }

  public static void buildMatrix(BitArray dataBits, int maskPattern, ByteMatrix matrix,QRCode qrCode) throws WriterException {
	  clearMatrix(matrix);
	  embedBasicPatterns(qrCode.getVersion(), matrix);
	    // Type information appear with any version.
	  embedTypeInfo(qrCode.getECLevel(), qrCode.getMaskPattern(), matrix);
	    // Version info appear if version >= 7.
	  maybeEmbedVersionInfo(qrCode.getVersion(), matrix);
    embedDataBits(dataBits, maskPattern, matrix);//将数据比特进行掩码并填充矩阵
  }
  ...
  public static void embedDataBits(BitArray dataBits, int maskPattern, ByteMatrix matrix)
    ...
    int y = matrix.getHeight() - 1;//以左上角为坐标原点，横轴为x，纵轴为y
    while (x > 0) {
      ...
      while (y >= 0 && y < matrix.getHeight()) {
        for (int i = 0; i < 2; ++i) {
          ...
          boolean bit;
          if (bitIndex < dataBits.getSize()) {//获取数据比特,以版本6为例getSize=1376=172*8
...
//              bit = MaskUtil.getDataMaskBit(maskPattern, xx, y);
          }
          matrix.set(xx, y, bit);
        }
        y += direction;
