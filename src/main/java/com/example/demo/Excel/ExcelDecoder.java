...
import com.example.demo.Excel.GeneralHelper.LtDecoder;
import com.example.demo.Excel.GeneralHelper.Sampler;
...
//                ((a,b)->(b.getValue().replaceAll("[^A-Za-z0-9]", "").length()-a.getValue().replaceAll("[^A-Za-z0-9]", "").length()));
        ...
        if (CyclicCoder.decode_without_correction(decodeInt,isNormal,MODE) != -1) {
...
//        WatermarkUtils watermarkUtils = new WatermarkUtils(new File(filePath));
        ...
        if(data.size()>0) {
            ...
            return StrBinaryTurn.stream2String(res);
//                        //分别保存中英文的可能结果
//                        secret_data.add(res.get(0));
//                        secret_data_chinese.add(res.get(1));
        }
