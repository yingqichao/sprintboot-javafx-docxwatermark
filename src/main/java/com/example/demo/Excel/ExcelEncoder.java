...
import com.example.demo.Excel.ExcelWatermarkHelper.excel.ExcelVersion;
...
import com.example.demo.Excel.ExcelWatermarkHelper.excel.ExcelUtil;
...
import com.example.demo.Excel.Utils.*;
...
//    }
    ...
    public ExcelEncoder(String f_bytes,String file,double max_allowed_modification,boolean isLong) {
        ...
        if(this.fileVersion.equals(".csv")){
            // CSV
            csvData = CsvUtil.readCsv(this.file.getPath());
        }else{
            //EXCEL
...
//                totalLen += validLen;
                if (validLen >= Settings.DEFAULT_MINLEN_EXCEL)
...
//            this.solitionGenerator.setK(Settings.rand_K);
        ...
        String crc_text = Util.dec2bin(CyclicCoder.encode(block_data,isNormal,MODE),(!isNormal&&MODE)?Settings.DEFAULT_EMBEDLEN_LONGMODE:Settings.DEFAULT_EMBEDLEN);
